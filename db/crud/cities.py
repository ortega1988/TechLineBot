...
async def get_cities_by_branch(session: AsyncSession, branch_id: int) -> Sequence[City]:
    result: ScalarResult[City] = (
        await session.execute(select(City).where(City.branch_id == branch_id))
    ).scalars()
    return result.all()


async def create_city(
    session: AsyncSession, name: str, url: str, branch_id: int
) -> City:
    city = City(name=name, url=url, branch_id=branch_id)
    session.add(city)
...
async def get_cities_by_branch_id(
    session: AsyncSession, branch_id: int
) -> Sequence[City]:
    result = await session.execute(
        select(City).where(City.branch_id == branch_id).order_by(City.name)
    )
    return result.scalars().all()
