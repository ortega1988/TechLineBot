...
from db.crud.cities import create_city, get_city_by_name
from db.db import async_session
from fsm.states import AddCityFSM
from keyboards.inline import (
    get_admin_menu,
    get_confirm_add_city_keyboard,
...
@router.callback_query(
    AddCityFSM.waiting_for_region, F.data.startswith("add_city_region_")
)
...
async def process_city_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Вставьте ссылку на город в 2ГИС (например: https://2gis.ru/kazan):"
    )
...
async def confirm_city(callback: CallbackQuery, state: FSMContext):
    ...
    async with async_session() as session:
        await create_city(
            session, name=data["name"], url=data["url"], branch_id=data["branch_id"]
        )
    await callback.message.edit_text(
        "✅ Город успешно добавлен!", reply_markup=get_admin_menu()
    )
    await state.clear()
    await callback.answer()
