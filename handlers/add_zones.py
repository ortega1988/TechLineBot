...
from db.crud.zones import create_zone, get_zone_by_name_and_city
...
async def add_zone_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Выберите филиал:", reply_markup=get_branches_keyboard(branches)
    )
...
@router.callback_query(
    AddZoneFSM.waiting_for_branch, F.data.startswith("add_zone_branch_")
)
async def select_branch(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Выберите город:", reply_markup=get_cities_keyboard(cities)
    )
...
async def select_city(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "Выберите ГКС:", reply_markup=get_areas_keyboard(areas)
    )
