...
from db.crud.branches import create_branch, get_branch_by_id, get_branch_by_name
from db.db import async_session
from fsm.states import AddBranchFSM
from keyboards.inline import get_admin_menu, get_confirm_add_branch_keyboard
...
async def start_add_branch(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer(
        "Введите <b>ID и название филиала</b> в одну строку (например: <b>16 Казанский</b>):"
    )
...
async def process_branch_id_and_name(message: Message, state: FSMContext):
    ...
    if len(parts) < 2:
        await message.answer(
            "⚠️ Введите ID и название через пробел (например: <b>16 Казанский</b>)"
        )
        return
    try:
...
async def confirm_branch(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "✅ Филиал успешно добавлен!", reply_markup=get_admin_menu()
    )
    await state.clear()
    await callback.answer()
